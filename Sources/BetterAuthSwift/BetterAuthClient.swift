...
public final class BetterAuthClient {
    ...
    @discardableResult
    public func signInWithApple(presentationAnchor: ASPresentationAnchor? = nil) async throws -> APIResponse<AuthData> {
        #if canImport(AuthenticationServices)
        let provider = AppleSignInTokenProvider(presentationAnchor: presentationAnchor)
        return try await signIn(with: provider, providerName: "apple")
        #else
    ...
    public func signIn(with provider: SignInTokenProvider, providerName: String) async throws -> APIResponse<AuthData> {
        ...
        return try await postSignIn(provider: providerName, body: body)
    }
    ...
    private func postSignIn<B: Encodable>(provider: String, body: B) async throws -> APIResponse<AuthData> {
